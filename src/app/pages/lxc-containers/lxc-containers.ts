...
@Component({
  selector: 'app-lxc-containers',
  imports: [RouterLink],
  templateUrl: './lxc-containers.html',
  styleUrl: './lxc-containers.scss',
})
export class LxcContainers implements OnInit, OnDestroy {

  private readonly socket = inject(VmSocketService);

  private socketSubscription?: Subscription;

  LxcContainers = signal<LxcContainerSummary[]>([]);
  ...
  errorMessage = signal<string | null>(null);
    ...
    loadVmsSocket(): void {
      ...
      this.socketSubscription = this.socket.getAllLxc().subscribe({
        next: (data: LxcContainerSummary[]) => {
          this.LxcContainers.set(data);
